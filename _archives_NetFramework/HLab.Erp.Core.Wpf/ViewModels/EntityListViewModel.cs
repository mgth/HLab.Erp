...
using HLab.Erp.Monographs.Module.Tools.Details;
using HLab.Mvvm;
...
namespace HLab.Erp.Core.Wpf.ViewModels
{
        ...
        where T : class, IEntity, new()
    {
        ...
        protected EntityListViewModel()
        {
            ...
            H.Initialize((TClass)this);
        }

        public ICommand OpenCommand => _openCommand.Get();
        private readonly IProperty<ICommand> _openCommand = H.Property<ICommand>(c => c
            .On(e => e.Selected)
            .Command(e => e._docs.OpenDocument(e.Selected), e=>e.Selected!=null)
        );
