...
namespace HLab.Erp.Data
{
    ...
    public class DataService : IDataService
    {
        ...
        public List<T> FetchQuery<T>(Func<IQueryable<T>, IQueryable<T>> q)
            where T : class, IEntity
        {
            //TODO : mysql exception quand server sql non accessible
            return Fetch(db =>
            {
                //TODO
                //var dbset = db.Query<T>( ).Where();

                //return q(dbset).ToList();
                return new List<T>();
            });
        }
            ...
            where T : class, IEntity
        {
            using (var db = Get())
            {
                ...
                var result = db.Query<T>().Where(expression).First();
        ...
        private DataCache<T> GetCache<T>() where T : class, IEntity
            => (DataCache<T>)_caches.GetOrAdd(typeof(T), t => new DataCache<T>() { DatabaseFactory = Get });
