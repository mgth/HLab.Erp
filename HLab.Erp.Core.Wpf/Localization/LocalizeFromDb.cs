...
{
   public class Bootloader(IDataService data, LocalizeFromDb localizeFromDb, ILocalizationService localization)
      : HLab.Core.Annotations.Bootloader
   {
      protected override BootState Load()
      {
         //if (bootstrapper.WaitingForService(data)) return Task.CompletedTask;

         localization.Register(localizeFromDb);
         return base.Load();
      }
   }
